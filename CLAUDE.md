...
- **`sudo mas uninstall` can't remove an App Store app that carries `com.apple.macl`**, which most do: it fails with `Failed to change ownership … Operation not permitted`, even with App Management granted. Finder can, so the removal task follows up by quitting the app and having Finder move it to the Bin. `No installed apps with ADAM ID` is also non-fatal, because `mas list` reads Spotlight and can lag a removal; any other error still fails the run.
